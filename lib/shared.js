(function(exports) {
		...
		try {
			...
			for (let index = 0; index < claims.length; index++) {
				...
				if (opType === 'claim_reward_balance') {
					...
					var sbdValue = 0.0;
					var tradeText = "";
					if (op[1].current_owner === accountName) {
						// account sells (current_owner)
						if (op[1].current_pays != undefined && op[1].current_pays.indexOf('STEEM') >= 0) {
							 // sell "STEEM";
							steemValue = getFloatValue(op[1].current_pays, -1.0);
						} else if (op[1].current_pays != undefined && op[1].current_pays.indexOf('SBD') >= 0) {
							// sell "SBD";
							sbdValue = getFloatValue(op[1].current_pays, -1.0);
						}
						if (op[1].open_pays != undefined && op[1].open_pays.indexOf('STEEM') >= 0){
							// buy "STEEM";
							steemValue = getFloatValue(op[1].open_pays, 1.0);
						} else if (op[1].open_pays != undefined && op[1].open_pays.indexOf('SBD') >= 0){
							// buy "SBD";
							sbdValue = getFloatValue(op[1].open_pays, 1.0);
						}
						tradeText = 'sell to ' + op[1].open_owner;	
					} else {
						// account buys (open_owner)
						if (op[1].current_pays != undefined && op[1].current_pays.indexOf('STEEM') >= 0) {
							// buy "STEEM";
							steemValue = getFloatValue(op[1].current_pays, 1.0);
						} else if (op[1].current_pays != undefined && op[1].current_pays.indexOf('SBD') >= 0) {
						...
						tradeText = 'buy from ' + op[1].current_owner;
					}
					// buy, cur, sell, cur
					var preparedDataObject = createPreparedDataObject(
						claim[0],
						...
						null,
						tradeText,
						op[1].current_owner, op[1].open_owner,
						claim[1],
						fnGetSteemPerMVestForDate,
						fnGetExchangeRate);
					preparedData[opType].push(preparedDataObject);
					...
					// TODO
					console.log(opType + ": " + op[1]);
				} else {
					// not handled transactions
					if (notHandledTransactions[opType] === undefined) {
						notHandledTransactions[opType] = 1;
					} else {
						notHandledTransactions[opType]++;
					}
				}
			}
			// log notHandledTransactions
			Object.keys(notHandledTransactions).forEach(function(key, index) {
				console.log("not prepared: " + key + " (" + this[key] + ")");
			  }, notHandledTransactions);
			return preparedData;
		} catch (error) {
			console.error(error);
		}
			...
			try {
				...
				for (let keyIndex = 0; keyIndex < preparedDataKeys.length; keyIndex++) {
					...
					for (let index = 0; index < preparedData[key].length; index++) {
						...
						htmlFragement += '<td class="text-right bg-secondary text-white">' + (element.amount_vests  != null && element.amount_vests != undefined ? element.amount_vests.toFixed(3) : '') + '</td>';
		...
		try {
			...
			for (let keyIndex = 0; keyIndex < preparedDataKeys.length; keyIndex++) {
				...
				for (let index = 0; index < preparedData[key].length; index++) {
					...
					output += (element.amount_steem != null && element.amount_steem != undefined ? element.amount_steem.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_steem != null && element.steem_btc_exchange != undefined && element.steem_btc_exchange != null ? element.steem_btc_exchange.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_steem != null && element.steem_btc != undefined && element.steem_btc != null ? element.steem_btc.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_steem != null && element.btc_eur_exchange != undefined && element.btc_eur_exchange != null ? element.btc_eur_exchange.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_steem != null && element.steem_eur != undefined && element.steem_eur != null ? element.steem_eur.toFixed(3).replace('.',decimalSeparator) : '') + ';';
	
					output += (element.amount_sbd  != null && element.amount_sbd != undefined ? element.amount_sbd.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sbd  != null && element.sbd_btc_exchange != undefined && element.sbd_btc_exchange  != null ? element.sbd_btc_exchange.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sbd  != null && element.sbd_btc != undefined && element.sbd_btc  != null ? element.sbd_btc.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sbd  != null && element.btc_eur_exchange != undefined && element.btc_eur_exchange  != null ? element.btc_eur_exchange.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sbd  != null && element.sbd_eur != undefined && element.sbd_eur  != null ? element.sbd_eur.toFixed(3).replace('.',decimalSeparator) : '') + ';';
	
					output += (element.amount_vests  != null && element.amount_vests != undefined ? element.amount_vests.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_vests  != null && element.steem_per_mvests != undefined && element.steem_per_mvests  != null ? element.steem_per_mvests.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sp  != null && element.amount_sp != undefined ? element.amount_sp.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sp  != null && element.vests_btc_exchange != undefined && element.vests_btc_exchange  != null ? element.vests_btc_exchange.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sp  != null && element.vests_btc != undefined && element.vests_btc  != null ? element.vests_btc.toFixed(6).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sp  != null && element.btc_eur_exchange != undefined && element.btc_eur_exchange  != null ? element.btc_eur_exchange.toFixed(3).replace('.',decimalSeparator) : '') + ';';
					output += (element.amount_sp  != null && element.vests_eur != undefined && element.vests_eur  != null ? element.vests_eur.toFixed(3).replace('.',decimalSeparator) : '') + ';';
	...
	function createCointrackingCsvRow(pType, pBuyAmount, pBuyCur, pSellAmount, pSellCur, pFeeAmount, pFeeCur, pGroup, pComment, pDate) {
		var exchange = "steemit.com";
		return `"${pType}","${pBuyAmount}","${pBuyCur}","${pSellAmount}","${pSellCur}","${pFeeAmount}","${pFeeCur}","${exchange}","${pGroup}","${pComment}","${pDate}"\n`;						
	}
	...
	function createCointrackingSteemitCurCsvRow(pType, pAmount, pCur, pFactor, pGroup, pComment, pDate) {
		...
		return createCointrackingCsvRow(pType, buyAmount, buyCur, sellAmount, sellCur, feeAmount, feeCur, pGroup, pComment, pDate);
	}
			...
			for (let keyIndex = 0; keyIndex < preparedDataKeys.length; keyIndex++) {
				...
				for (let index = 0; index < preparedData[key].length; index++) {
					...
					if (element.op_type === "claim_reward_balance" || element.op_type === "author_reward") {
						...
						output += createCointrackingSteemitCsvRow(cointrackingTyp, element.amount_steem, element.amount_sbd, factor, element.op_type, comment, element.timestamp.replace('T',' '));
					} else if (element.op_type === "fill_order") {
						cointrackingTyp = "Trade";
						var buyAmount = 0.0;
						var buyCur = "";
						var sellAmount=0.0;
						var sellCur = "";
						if (element.amount_steem > 0.0) {
							// buy = positive value
							buyAmount = element.amount_steem;
							buyCur = "STEEM";
						} else if (element.amount_steem < 0.0) {
							// sell == negative value
							sellAmount = element.amount_steem * (-1);
							sellCur = "STEEM";
						}
						if (element.amount_sbd > 0.0) {
							// buy = positive value
							buyAmount = element.amount_sbd;
							buyCur = "SBD";
						} else if (element.amount_sbd < 0.0) {
							// sell == negative value
							sellAmount = element.amount_sbd * (-1);
							sellCur = "SBD";
						}
						output += createCointrackingCsvRow(
							cointrackingTyp, 
							buyAmount, buyCur, 
							sellAmount, sellCur,
							"", "", 
							element.op_type, comment, element.timestamp.replace('T',' '));
					} else {
						console.log("createCointrackingCsvOutput not handled: " + element.op_type + "\n" + element);
					}
				}
			}
			return output;
